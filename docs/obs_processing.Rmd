...
**TODO: Vince to add general info about the role of o-b surveys**

In the Triangle, a survey of GoTriangle and DCHC transit agencies was collected
in 2014 while GoTriangle and CAMPO agencies were collected in 2015.

## Combining surveys

Unlike the household surveys, the two transit surveys were collected by different
...
destination, boarding, and alighting locations.
...
As a check on both the on-board survey processing and the model's route network,
the on-board survey was assigned. This involved processing the survey into the
appropriate matrices by time of day, transit mode, and accesss mode. The model
assignment macros in TransCAD were used to assign these to the route system, and
the results were compared to reported ridership by route.
...
temp <- add_mode %>%
  mutate(
    access_mode = case_when(
      access_mode %in% c("walk", "bike") ~ "w",
      grepl("pnr", access_mode) ~ "pnr",
      grepl("knr", access_mode) ~ "knr",
      TRUE ~ "other"
    ),
    mode = ifelse(mode == "local", "lb", "eb")
  ) %>%
...
# Metrics::rmse(comparison$model_boardings, comparison$reported) / mean(comparison$reported)
```


```{r, eval=FALSE}
write_csv(add_mode, "data/output/_PRIVATE/obs_processing/obs_processed.csv")
