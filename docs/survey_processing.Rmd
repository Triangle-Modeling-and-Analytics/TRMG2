...
Similar to the person table, the trip tables from 2016 and 2018 contained some
different fields and field definitions. Importantly, the definition of activity
purpose changed slightly between the two years as shown in the table below.

```{r}
tibble(
  `Activity Code` = c(2, 3),
  `2016` = c("Working (for pay) at work or home", "Work-related: delivering goods or services"),
  `2018` = c("At home, working for pay", "At work (not home), working for pay"),
) %>%
  kable() %>%
  kable_styling()
```

The other codes remained the same. The change to 2018 makes it easier to identify
people working from home, but most records are still from 2016 and require using
several fields to infer work from home.
...
convert_purpose <- trip_combined %>%
  left_join(activity_2016, by = c("o_activity_2016" = "code")) %>%
  rename(o_activity1 = activity) %>%
  left_join(activity_2016, by = c("d_activity_2016" = "code")) %>%
  rename(d_activity1 = activity) %>%
  left_join(activity_2018, by = c("o_activity_2018" = "code")) %>%
  rename(o_activity2 = activity) %>%
  left_join(activity_2018, by = c("d_activity_2018" = "code")) %>%
  ...
  select(-c(
    o_activity1:d_activity2
    # o_activity_2016, o_activity_2018,
    # d_activity_2016, d_activity_2018
  )) %>%
  mutate(
    mode = case_when(
      mode == 1 ~ "auto_own",
...
determine_tours <- determine_wfh %>%
  ...
  mutate(
    temp = ifelse(d_activity == "W", "W", ""),
    temp = ifelse(d_activity == "WFH", "WFH", temp),
    temp = ifelse(tripnum == max(tripnum) & temp == "WFH", "", temp),
    temp = ifelse(o_activity %in% c("H", "WFH") & d_activity %in% c("H", "WFH"), "H", temp),
    tour_type = case_when(
      "W" %in% temp ~ "W",
      "WFH" %in% temp ~ "WFH",
      "H" %in% temp ~ "H",
  ...
  select(-temp, -new_tour)

# determine_tours %>%
#   select(
#     personid, tripnum, o_activity, d_activity, tour_num, tour_type#,
#     # temp, new_tour
#   ) %>%
#   View()
```
