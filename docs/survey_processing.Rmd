...
determine_wfh <- calc_distances %>%
  mutate(
    ...
    d_test = d_activity,
    o_test = ifelse(
      o_test == "W?" & dist_orig_home < 100, "WFH", o_test),
    o_test = ifelse(o_test == "W?", "W", o_test),
    d_test = ifelse(
      d_test == "W?" & dist_dest_home < 100, "WFH", d_activity),
    d_test = ifelse(d_test == "W?", "W", d_test)
  )


write_csv(determine_wfh, "test.csv")
```
