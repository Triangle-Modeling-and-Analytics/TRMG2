...
is much simpler. Rather than requiring detailed tour pattern information to 
support coordinated activity patterns within a household as in activity-based
models, tours can be classified simply as work or non-work.
...
prep_for_eda <- add_se_data %>%
  mutate(homebased = ifelse(p_place == "H", "HB", "NHB")) %>%
  left_join(
    hh_add_seniors %>%
      select(
        hhid, hhsize, num_vehicles, num_children, child_in_hh,
        hh_income_broad, num_seniors, senior_in_hh, num_workers
      ),
    by = "hhid"
  ) %>%
  mutate(
  ...
  mutate(
    ...
    senior_present = ifelse(num_seniors > 0, 1, 0),
    num_workers = sum(is_worker, na.rm = TRUE),
    worker_present = ifelse(num_workers > 0, 1, 0)
  ) %>%
  ungroup() %>%
  relocate(num_seniors:worker_present, .after = child_in_hh) %>%
  mutate(
...
```{r}
# EDA based on person-level attributes

prep_per_tbl <- per_add_flags %>%
  left_join(
    hh_add_seniors
  )

# person_eda_scheme2 <- person_eda(scheme2)
```
...
write_csv(
  person_eda_scheme2,
  "data/output/survey_processing/person_eda_scheme2.csv",
  na = ""
)
```
