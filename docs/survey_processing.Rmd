...
# TODO: use "LL" and "SS" etc for the NT/NHB trips

# This step adds the first purpose scheme defined by Vince
scheme1 <- prep_for_eda %>%
  mutate(
    purp_scheme1 = d_activity,
    dur_scheme1 = duration,
    # WT HB trips
    purp_scheme1 = case_when(
      tour_type != "WT" ~ purp_scheme1,
      homebased != "HB" ~ purp_scheme1,
      purp_scheme1 %in% c("POS", "DOS") ~ "SCH",
      purp_scheme1 %in% c("W", "SCH") ~ purp_scheme1,
      TRUE ~ "O"
    ),
    dur_scheme1 = case_when(
      tour_type != "WT" ~ dur_scheme1,
      homebased != "HB" ~ dur_scheme1,
      purp_scheme1 %in% c("W", "SCH") ~ "All",
      TRUE ~ dur_scheme1
    ),
    # WT NHB trips
    purp_scheme1 = case_when(
      tour_type != "WT" ~ purp_scheme1,
      homebased != "NHB" ~ purp_scheme1,
      o_activity %in% c("POS", "DOS") ~ "SCH",
      d_activity %in% c("POS", "DOS") ~ "SCH",
      o_activity == "SCH" | d_activity == "SCH" ~ "SCH",
    ...
    dur_scheme1 = case_when(
      tour_type != "WT" ~ dur_scheme1,
      homebased != "NHB" ~ dur_scheme1,
      purp_scheme1 %in% c("WR", "SCH", "EAT") ~ "All",
      TRUE ~ dur_scheme1
    ),
    # NT HB trips
    purp_scheme1 = case_when(
      tour_type != "NT" ~ purp_scheme1,
      homebased != "HB" ~ purp_scheme1,
      purp_scheme1 %in% c("POS", "DOS") ~ "SCH",
      purp_scheme1 %in% c("SCH", "SHP", "EAT", "OM", "OMED") ~ purp_scheme1,
      TRUE ~ "OD"
    ),
    dur_scheme1 = case_when(
      tour_type != "NT" ~ dur_scheme1,
      homebased != "HB" ~ dur_scheme1,
      purp_scheme1 %in% c("SCH", "EAT", "OM", "OMED") ~ "All",
      TRUE ~ dur_scheme1
    ),
    # NT NHB trips
    purp_scheme1 = case_when(
    ...
    dur_scheme1 = case_when(
      tour_type != "NT" ~ dur_scheme1,
      homebased != "NHB" ~ dur_scheme1,
      purp_scheme1 %in% c("EAT-SHP", "EAT-O") ~ "All",
      TRUE ~ dur_scheme1
    ),
  ) %>%
  relocate(c(tour_type, homebased), .before = purp_scheme1)




eda_scheme1 <- scheme1 %>%
  group_by(tour_type, homebased, purp_scheme1, dur_scheme1) %>%
  nest() %>%
  mutate(
    wTrips = map_dbl(
  ...
  select(-data) %>%
  filter(tour_type != "H") %>%
  arrange(tour_type, homebased, purp_scheme1, dur_scheme1)
```
