...
- WFH: Work from home
- W: Work outside the home
- WR: Work-related (e.g. making a delivery)
- Sch: School
- Shp: Shop
- Eat: Dining out
- PUS: Pick up from school
- PUO: Pick up from other
- DOS: Drop off at school
- DOO: Drop off at other
- O: Other
...
to determine which work activities were performed at home. The same procedure
was used to determine if pick-up/drop-off trips were to school or not.
...
```{r determine_wfh}
# Calculate distances of origins and destinations from home
calc_distances <- convert_purpose %>%
  left_join(
    hh_combined %>% select(hhid, home_loc_lat, home_loc_lng),
    by = "hhid"
  )
o_dists <- distHaversine(
  calc_distances %>% select(origin_lng, origin_lat),
  calc_distances %>% select(home_loc_lng, home_loc_lat)
)
d_dists <- distHaversine(
  calc_distances %>% select(destination_lng, destination_lat),
  calc_distances %>% select(home_loc_lng, home_loc_lat)
)

determine_wfh <- calc_distances %>%
  mutate(
    dist_orig_home = o_dists,
    dist_dest_home = d_dists,
    o_activity = ifelse(
      o_activity == "W?" & dist_orig_home < 100, "WFH", o_activity),
    o_activity = ifelse(o_activity == "W?", "W", o_activity),
    d_activity = ifelse(
      d_activity == "W?" & dist_dest_home < 100, "WFH", d_activity),
    d_activity = ifelse(d_activity == "W?", "W", d_activity)
  )
```

```{r determine_pudo_school}
...
school_loc <- per_combined %>%
  select(hhid, school_lat, school_lng) %>%
  filter(!is.na(school_lat))

determine_pudo_sch <- determine_wfh %>%
  left_join(school_loc, by = "hhid") %>%
  mutate(
    dist_orig_sch = calc_dist(origin_lng, origin_lat, school_lng, school_lat)
  ) %>%
  group_by(tripid) %>%
  arrange(tripid, dist_orig_sch) %>%
  slice(1) %>%
  ungroup() %>%
  select(-school_lat, -school_lng) %>%
  left_join(school_loc, by = "hhid") %>%
  mutate(dist_dest_sch = calc_dist(
    destination_lng, destination_lat, school_lng, school_lat
  )) %>%
  group_by(tripid) %>%
  arrange(tripid, dist_dest_sch) %>%
  slice(1) %>%
  ungroup() %>%
  mutate(
    o_activity = case_when(
      o_activity == "PU" & dist_orig_sch < 100 ~ "PUS",
      o_activity == "PU" ~ "PUO",
      o_activity == "DO" & dist_orig_sch < 100 ~ "DOS",
      o_activity == "DO" ~ "DOO",
      TRUE ~ o_activity
    ),
    d_activity = case_when(
      d_activity == "PU" & dist_dest_sch < 100 ~ "PUS",
      d_activity == "PU" ~ "PUO",
      d_activity == "DO" & dist_dest_sch < 100 ~ "DOS",
      d_activity == "DO" ~ "DOO",
      TRUE ~ d_activity
    )
  )
