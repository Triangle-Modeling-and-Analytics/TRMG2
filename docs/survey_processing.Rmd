...
trip_combined <- bind_rows(
  ...
  left_join(
    add_flags %>% select(personid, age, is_child, is_senior, is_worker, school),
    by = "personid"
  ) %>%
...
prep_for_eda <- add_se_data %>%
  ...
  ungroup() %>%
  relocate(num_seniors:worker_present, .after = child_present) %>%
  mutate(
