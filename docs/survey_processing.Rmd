...
scheme1 <- prep_for_eda %>%
  mutate(
    ...
    purp_scheme = case_when(
      ...
      purp_scheme %in% c("POS", "DOS") ~ "SCH",
      purp_scheme %in% c("W", "SCH") ~ purp_scheme,
      TRUE ~ "O"
    ),
    dur_scheme = case_when(
      tour_type != "WT" ~ dur_scheme,
      homebased != "HB" ~ dur_scheme,
      purp_scheme %in% c("W", "SCH") ~ "All",
      TRUE ~ dur_scheme
    ),
    # WT NHB trips
    purp_scheme = case_when(
    ...
    dur_scheme = case_when(
      ...
      purp_scheme %in% c("WR", "SCH", "EAT") ~ "All",
      TRUE ~ dur_scheme
    ),
    # NT HB trips
    purp_scheme = case_when(
      ...
      homebased != "HB" ~ purp_scheme,
      purp_scheme %in% c("POS", "DOS") ~ "SCH",
      purp_scheme %in% c("SCH", "SHP", "EAT", "OM", "OMED") ~ purp_scheme,
      TRUE ~ "OD"
    ),
    dur_scheme = case_when(
      tour_type != "NT" ~ dur_scheme,
      homebased != "HB" ~ dur_scheme,
      purp_scheme %in% c("SCH", "EAT", "OM", "OMED") ~ "All",
      TRUE ~ dur_scheme
    ),
    # NT NHB trips
    purp_scheme = case_when(
