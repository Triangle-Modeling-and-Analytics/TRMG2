...
per_combined <- bind_rows(
  ...
  left_join(
    hh_add_income_final %>% select(hhid, hh_weight_combined),
    by = "hhid"
  ) %>%
...
hh_add_seniors <- hh_add_income_final %>%
  left_join(
    per_add_flags %>%
      group_by(hhid) %>%
...
add_inc_groups <- add_obs_trip_type %>%
  left_join(
    hh_update_taz %>% select(hhid, hh_inc_final, hh_inc_final_lab),
    by = "hhid"
  )
```
