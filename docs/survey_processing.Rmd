...
In the table below, the "W?" purpose denotes a work activity that could either
be in or outside the home.
...
each were tagged to the appropriate place type. This approach gave a more
accurate result compared to depending on the names each person gave their origin
and destination, which varied widely.

Work location was done differently...**FILL IN**
...
Numerous problems detected with the coding of school activity purpose.

- School activities reported at locations other than school/home (850 origins / 876 dests)
  - Bus stops coded as school rather than mode transfer (70 origins / 70 dests)
  - PU/DO coded as school (300 origins / 300 origins)
  - Public libraries
...
determine_tours %>%
  group_by(personid, tour_num) %>%
  mutate(
    party_size_change = lead(party_size) - party_size,
    dest_act_dur = lead(departure_time) - arrival_time,
    do_flag = ifelse(
      d_activity == "SCH" &
        lead(personnum) == personnum &
        party_size_change < 0 &
        dest_act_dur < 15
        , 1, 0
    ),
    d_activity = ifelse(do_flag == 1, "DO", d_activity),
    o_activity = case_when(
      is.na(lag(do_flag)) ~ o_activity,
      lag(do_flag) == 1 ~ "DO",
      TRUE ~ o_activity
    )
  ) %>%
