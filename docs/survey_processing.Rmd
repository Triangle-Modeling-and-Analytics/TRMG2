...
accurate result than depending on the names each person gave their origin
...
school_loc <- per_combined %>%
  ...
  select(-hhid, -school_address, -home_loc_address)
...
det_sch_place <- det_home_place %>%
  ...
  mutate(
    ...
    is_student = ifelse(is.na(dest_sch_dist), 0, 1),
    o_place = case_when(
...
- School activities reported by non-students (1000 trips)
  - School activities reported at locations other than school/home (850 trips)
    - Public libraries
  ...
  mutate(
    ...
    bus_flag = ifelse(
      str_detect(tolower(destination_name), "bus") &
        str_detect(tolower(lead(origin_name)), "bus"),
      1, 0
    ),
    do_flag = ifelse(
      d_activity == "SCH" &
