...
per_combined <- bind_rows(
  ...
  left_join(
    hh_add_flags %>% select(hhid, hh_weight_combined),
    by = "hhid"
  ) %>%
...
- SCH: School. Separated using age in the person table to:
...
The table below shows how the survey mode field was simplified into the model
modes. For the auto mode, occupancy was used to split into single-occupancy
(sov) and high-occupancy (hov).
...
For each person in the survey, a tour begins with their first trip and ends when
they return home. A second tour starts if they leave home again. Most tours are
classified as work if the traveler has a work activity during the tour. If they
do not, then the tour is classified as non-work. A small number of tours have a
work activity during the tour, but the duration of that activity is short (less
than 2 hours) or they spend twice as long somewhere else as at work. These tours
are classified as non-work.
...
purpose of travel modeling, these home tours are ignored in TRMG2; however, 
this segmentation scheme would allow for the addition of non-motorized "loop"
tours without any reported stops for improved non-motorized trip modeling in
a future version of the model without requiring any changes to the existing 
trip purposes. 
...
temp <- flag_loop_trips %>%
  filter(tour_type == "WT", d_activity == "W") %>%
  group_by(personid, tour_num) %>%
...
temp2 <- flag_loop_trips %>%
  filter(tour_type == "WT") %>%
  group_by(personid, tour_num) %>%
  mutate(max_o_duration = max(d_act_dur, na.rm = TRUE)) %>%
...
flip_to_nt <- flag_loop_trips %>%
  ...
  group_by(personid, tour_num) %>%
  mutate(tour_type = ifelse(flip_to_nt == 1, "NT", tour_type))
```
...
There were numerous problems detected in the survey. These include:
...
These problems were identified and cleaned such that each person's trip diary
was internally logically consistent with itself and externally consistent with
the diaries of other household members. 
...
stops. The survey treated parking location as an attribute of the trip rather an
origin or destination, which meant no processing was required, rather linking 
for this mode transfer was included in the delivered survey data.
...
- `Is Child (r)`: Correlation between trip making and being <18
- `Employment (r)`: Correlation between trips attracted to a zone and employment
- `AM (%)`: Percentage of trips in the AM period
- `HOV (%)`: Percentage of trips using the high-occupancy vehicle mode
- `Trip Length`: Mean trip length

```{r}
prep_for_eda <- add_se_data %>%
  mutate(
    homebased = ifelse(p_place == "H", "HB", "NHB"),
    ...
    tod = case_when(
      ...
      departure_time < (3 + 12)*60 ~ "MD",
      departure_time < (6 + 12)*60 ~ "PM",
      TRUE ~ "NT"
    )
  )
```
...
scheme1 <- prep_for_eda %>%
  mutate(
    ...
    purp_scheme = case_when(
      ...
      a_activity %in% c("POS", "DOS") ~ "K12",
      o_activity == "K12" | a_activity == "K12" ~ "K12",
...
scheme6 <- prep_for_eda %>%
  mutate(
    ...
    purp_scheme = case_when(
      ...
      a_activity == "K12" | p_activity == "K12" ~ "K12-O/OME",
      TRUE ~ "O-O"
    ),
    dur_scheme = case_when(
      tour_type != "NT" ~ dur_scheme,
