...
trips <- trips_orig %>%
  mutate(
    # mode_nhb = case_when(
    #   homebased == "HB" ~ mode_final,
    #   mode_final != "other_auto" ~ mode_final,
    #   party_size == 1 ~ "sov",
    #   party_size == 2 ~ "hov2",
    #   party_size >= 3 ~ "hov3"
    # ),
    # trip_type = paste0(trip_type, "_", mode_nhb)
    trip_type = paste0(trip_type, "_", mode_final)
  ) %>%
...
coeff_tbl <- model3$tbl %>%
  add_row(term = "r_sq", estimate = zonal_result$r_sq)
coeffs_list[gsub("_lb", "_t", type)] <- list(coeff_tbl)
make_table(coeff_tbl)
p_tbl_list[type] <- list(model3$p_tbl)
```
...
No significant presence of this trip type in the survey (21 samples), and it is
not an important source of travel demand. No model was estimated.
...
equiv <- c(
  ...
  "N_HB_OD_Short_hov2" = "N_HB_OD_All_hov",
  "N_HB_OD_Short_hov3" = "N_HB_OD_All_hov",
  "N_HB_OD_Short_lb" = "N_HB_OD_All_lb",
  "N_HB_OD_Long_lb" = "N_HB_OD_All_lb",
...
After applying the estimated model, Caliper calculated calibration factors such
that total NHB trips matched the expanded survey. Factors were calculated by
tour type and mode. A coarser approach, such as calibrating a single factor for
all NHB trips, would leave rarer modes (e.g. transit) with either too few or
to many trips.

Some of the calibration factors are larger than those seen in other models (e.g.
homebased generation). This is the result of two features of this modeling
approach. Firstly, recall that the estimated models do not predict chained NHB
trips (see previous section). While the spatial distribution of predicted trips
is representative, the totals must be scaled up to account for these additional
trips.

Secondly, the scaling of trip productions by zonal accessibility also impacts
the trip totals. There is no guarantee, for example, but that the number of
trips scaled up and down will match or that the original total will be
preserved. The impact is especially large for transit. No boosting coefficients
could be estimated from the survey, but the model enforces logical consistency
by removing transit NHB trip predicted where no transit access exists. This
requires that the remaining NHB transit trips be scaled up to compensate.

For these reasons, these calibration factors are acceptable as long as
assignment results and distribution patterns look reasonable.

```{r, include=FALSE}
results <- read_csv("data/input/nhb/estimated_model_app_results.csv") %>%
...
totals <- results %>%
  ...
  pivot_longer(everything(), names_to = "type", values_to = "trips")
...
comparison <- trips %>%
  ...
  group_by(tour_type, mode_nhb) %>%
  summarize(obs_trips = sum(trip_weight_combined)) %>%
  unite(type, c(tour_type, mode_nhb), sep = "_") %>%
  left_join(totals, by = "type") %>%
  mutate(
    diff = trips - obs_trips,
    pct_diff = (diff / obs_trips) * 100,
    factor = obs_trips / trips
  )
...
comparison %>%
  ...
  mutate(
    ...
    trips = round(trips, 0),
    tour_type = substring(type, 1, 1),
    mode = substring(type, 3, length(type)),
    mode = case_when(
      mode == "auto_pay" ~ "Auto Pay",
      mode == "sov" ~ "SOV",
      mode == "hov2" ~ "HOV2",
      mode == "hov3" ~ "HOV3",
      mode == "t" ~ "Transit",
  ...
  select(tour_type, mode, factor) %>%
  pivot_wider(names_from = tour_type, values_from = factor) %>%
  arrange(mode) %>%
  rename(
    Mode = mode,
    `NonWork Tour` = N,
...
write_csv(calib_out_tbl, "data/output/nhb/nhb_calibration_factors.csv")
```