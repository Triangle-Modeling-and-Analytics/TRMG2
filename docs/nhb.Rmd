...
all NHB trips, would leave rarer modes like transit with either too few or
to many trips.
...
trip totals. There is no guarantee, for example, that the number of trips
scaled up and down will match or that the original total will be preserved. The
impact is especially large for transit. No boosting coefficients could be
estimated from the survey, but the model enforces logical consistency by
removing transit NHB trip predicted where no transit access exists. This
