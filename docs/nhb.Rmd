...
```{r, W_NH_EK12_All_other_auto}
type <- "W_NH_EK12_All_other_auto"
n_records <- trips %>%
  filter(trip_type == type) %>%
  nrow()
```
...
```{r, W_NH_WR_All_other_auto}
type <- "W_NH_WR_All_other_auto"
...
# Restrict dependent variables
dependent_vars <- c(
  'W_HB_O_All_other_auto',
  'W_HB_O_All_sov',
  'W_HB_W_All_other_auto'
)
...
```{r, W_NH_O_All_other_auto}
type <- "W_NH_O_All_other_auto"
...
```{r, N_NH_K12_All_other_auto}
type <- "N_NH_K12_All_other_auto"
...
# Restrict dependent variables
dependent_vars <- c(
  'N_HB_OD_Long_other_auto',
  'N_HB_K12_All_other_auto',
  'N_HB_OD_Short_hov2',
  'N_HB_OD_Short_hov3',
  'N_HB_K12_All_school_bus',
  'N_HB_K12_All_hov2',
  'N_HB_OD_Short_other_auto',
...
```{r, N_NH_OME_All_other_auto}
type <- "N_NH_OME_All_other_auto"

# Collapse trip types
equiv <- list(
  "N_HB_OD_Long_hov2" = "N_HB_OD_Long_hov2",
  "N_HB_OD_Long_hov3" = "N_HB_OD_Long_hov2",
  "N_HB_OD_Short_hov2" = "N_HB_OD_Short_hov",
  "N_HB_OD_Short_hov3" = "N_HB_OD_Short_hov"
)
...
```{r, N_NH_O_All_other_auto}
type <- "N_NH_O_All_other_auto"
...
After applying the estimated model, Caliper checked the resulting trip totals
by mode against the survey.
...
comparison <- trips %>%
  ...
  mutate(mode_nhb = ifelse(mode_nhb %in% c("lb", "eb"), "t", mode_nhb)) %>%
  ...
  mutate(
    diff = obs_trips - trips,
    pct_diff = diff / obs_trips * 100
  )

write_csv(comparison, "nhb_calibration.csv")
```
