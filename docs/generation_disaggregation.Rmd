...
The first step in synthesizing the population of travelers in the TRMG2 is to 
...
households in each zones based on the average household size in the zone.  
...
some travel models' population synthesizers require the user to supply the 
...
Therefore, to keep the data development burden as light as possible, Caliper
has retained the approach of using stratification curves in the TRMG2.  These
curves simply encode information on the extent to which, for example, a zone
with 3.5 persons-per-household on average will have a high percentage of 3- 
and 4-person households and a zone with an average of 1.5 persons-per-
household will have more 1- and 2-person households.
...
between blockgroups and zones results in the distribution having different 
...
blockgroup based on the blockgroup's average household size. (Click on the 
siz2, siz3, and siz4 entries in the legend to the right of the chart to turn 
them off and focus on the green, 1-person households.) Multiple blockgroups 
that have the same average household size will usually have slight differences 
in their shares of 1-person households. For example, blockgroups with an 
average household size of 2.0 generally have 37-47% 1-person households; 
whereas, blockgroups with close to 3.5 persons per household on average 
...
household in the zonal data provided appears to be slightly inconsistent with 
the ACS data (reflecting slightly higherr unemployment or lower workforce 
participation as may have been the case earlier in the decade following the
Great Recession). However, the difference is small enough (1.2 vs 1.3 workers 
per household) that the Census data should be suitable for use. Moreover, 
presumably, if something were to be changed, it would be to make the zonal 
data more consistent with the Census data.  
...
Before the same treatment was applied for median income, Caliper had to determine appropriate income groups. ACS data is already grouped to some extent,
