...
To support proper handling in mode choice, the TNC services (e.g. Uber and Lyft)
need appropriate values for fare and initial wait times. Both were estimated as
functions of accessibility, and are calculated during this step of the model.

The linear regression model for fare was estimated from the survey and is shown
below.
...
The wait time (shown in a map above), is a function of general accessibility as
shown below:
...
In the peak period, 3.5 minutes are added to the wait time.