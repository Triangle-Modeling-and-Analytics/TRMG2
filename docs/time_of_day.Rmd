...
tod_factors <- tripTbl %>%
  filter(tour_type != "H") %>%
  group_by(Period = tod, Purpose = trip_type) %>%
...
tod_factors %>%
  ungroup() %>%
  mutate(Percent = percent(Percent, accuracy = .1)) %>%
  # filter(Purpose %in% c("W_HB_W_All", "N_HB_K12_All")) %>%
...
- sr3: Shared ride with 3+ people

Vehicle occupancies for da and sr2 modes are 1.0 and 2.0, respectively. The
table below shows the occupancies calculated for sr3.

```{r calculate veh occ factors}
occ_tbl <- tripTbl %>%
  filter(tour_type != "H", !is.na(mode_simple2)) %>%
  filter(mode_simple2 %in% c("sov", "hov")) %>%
  mutate(
