...
number of reasons in the model, but is not actually how travel occurs. As a
result, before highway trip assignment can take place, the PA format must be
converted to "Origin/Destination" (OD) format. In this format, a trip from work
to home starts at work and ends at home.
...
elsewhere. The table below shows the directionality factors calculated from
the survey.

```{r calc_pa_factors}
direction_tbl <- tripTbl %>%
  filter(homebased == "HB", tour_type != "H") %>%
  group_by(tour_type, tod, pa_flag) %>%
  summarize(trips = sum(trip_weight_combined)) %>%
  mutate(
    pct = trips / sum(trips),
    pa_flag = ifelse(pa_flag == 1, "PA", "AP"),
    pa_flag = factor(pa_flag, levels = c("PA", "AP"), ordered = TRUE),
    tod = factor(tod, levels = c("AM", "MD", "PM", "NT"), ordered = TRUE)
  ) %>%
  select(-trips) %>%
  arrange(tour_type, tod, pa_flag)

direction_tbl %>%
  pivot_wider(
    names_from = "tod", values_from = "pct"
  ) %>%
  mutate(tour_type = ifelse(tour_type == "N", "NonWork", "Work")) %>%
  rename(`Tour Type` = tour_type, Direction = pa_flag) %>%
  kable(digits = 2) %>%
  kable_styling(full_width = FALSE)
```
...
In the TRMG2, the directionality factors are also used to improve skims. In a
traditional model, an AM skim is often used for all peak trips while a MD skim
is used for off-peak. TRMG2 does independent feedback by time of day, which
required Caliper to adjust each period skim to account for directionality.

The easiest way to explain the adjustment is using the PM period as an example.
From the table above, 95% of work trips in the PM are traveling back home. At
the same time, trip productions estimated by the model (still in PA format)
see skim times from home to work, which are near free-flow speed. The simplest
treatment (and one done in Caliper's previous work) is to transpose the PM
skim. In this way, the trips in zone 1 actually see skim times for travel to
instead of from zone 1.

This simple treatment is adequate for some models, but the Triangle has a large
amount of non-work travel in the PM peak. The table above shows us that these
trips have a more balanced directionality. Many of these trips are traveling
in the less congested direction. As a consequence, Caliper created weighted
average skims using the PA/AP factors. For any ij pair of zones, the travel time
is computed as:

$$TT_{avg} = (f_{pa} * TT_{ij} + f_{ap} * TT_{ji}) / 2$$

Where:  
$TT_{avg}$ is the average travel time from zone i to j;
$f_{pa}$ is the PA factor;
$TT_{ij}$ is the travel time from zone i to j;
$f_{ap}$ is the AP factor; and
$TT_{ap}$ is the travel time from zone j to i.

With this approach, the travel time skim gives a balanced look at travel
impedance in the region and will improve destination and mode choice predictions.
