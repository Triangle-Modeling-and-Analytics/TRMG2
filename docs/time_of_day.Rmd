...
table below shows the occupancy calculated for HOV3, auto pay, and other
auto.
...
```{r}
# calculate for auto_pay and other_auto
oth_occ_tbl <- tripTbl %>%
  filter(tour_type != "H", !is.na(mode_final)) %>%
  ...
  # Low samples for some rows leads to high factors (e.g. 4.9). If the sample size
  # is low, use the daily average. If it is still too low, combine modes.
  group_by(trip_type) %>%
  mutate(purpose_daily_value = round(weighted.mean(party_size, w = trip_weight_combined), 3)) %>%
  group_by(trip_type, mode) %>%
  mutate(
    daily_value = round(weighted.mean(party_size, w = trip_weight_combined), 3),
    daily_samples = n(),
    ...
    Value = case_when(
      Samples > 30 ~ Value,
      daily_samples > 30 ~ daily_value,
      TRUE ~ purpose_daily_value
    )
  ) %>%
  group_by(trip_type, tod, mode) %>%
  slice(1) %>%
  select(trip_type, tod, mode, Value) %>%
  ungroup() %>%
  arrange(trip_type, tod, mode) %>%
  complete(mode, trip_type, tod) %>%
  group_by(trip_type) %>%
  mutate(Value = ifelse(is.na(Value), max(Value, na.rm = TRUE), Value)) %>%
  pivot_wider(names_from = mode, values_from = Value)

final_occ <- occ_tbl %>%
  left_join(oth_occ_tbl, by = c("trip_type", "tod"))
```
...
# Occupancy factors
write_csv(final_occ , "data/output/tod/veh_occ_factors.csv")
