...
The approach above is most appropriate in the peak periods to accurately capture
peak congestion. It is also fine in the midday, when traffic flow is relatively
even. However, due to the unique nature of the NT period, it generally
overstates congestion. For this reason, additional capacity is added. The final
period factors are shown below.

```{r}
extra_nt_cap <- pcf_tbl %>%
  mutate(PCF = ifelse(Period == "NT", 4, PCF))
