...
```{r}
# Calculate the shares of auto_pay and other_auto trips that are sov/hov2/hov3
trip_types <- tripTbl %>%
  filter(tour_type != "H", !is.na(mode_final)) %>%
  select(trip_type) %>%
  group_by(trip_type) %>%
  slice(1)
trip_types <- expand(trip_types, mode_final = c("auto_pay", "other_auto"))
...
write_csv(hov3_tbl , "data/output/tod/hov3_occ_factors.csv")
write_csv(oth_shares , "data/output/tod/other_shares.csv")
