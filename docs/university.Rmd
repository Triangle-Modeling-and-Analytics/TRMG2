...
#source("R/university_attractions2.R")
...
w_avg_trips_byres_df<-Person_subset_df%>%
  filter(!is.na(Weight))%>%
  mutate(w_trips=Derived_trip_rate * Weight)%>%
  group_by(On_campus)%>%
  summarize(w_total_trips=sum(w_trips),
...
on_campus_w_respondents<-w_avg_trips_byres_df %>%
  filter(On_campus==1)%>%
  select(w_respondents)%>%
  pull()

on_campus_sample<-w_avg_trips_byres_df %>%
  filter(On_campus==1)%>%
  select(respondents)%>%
  pull()

off_campus_w_respondents<-w_avg_trips_byres_df %>%
  filter(On_campus==0)%>%
  select(w_respondents)%>%
  pull()

off_campus_sample<-w_avg_trips_byres_df %>%
  filter(On_campus==0)%>%
  select(respondents)%>%
  pull()
```
...
```{r,echo=FALSE}
correlations_df %>% 
 select(term,W_Trips, W_On_Trips, W_Off_Trips) %>%
  kable(digits = 3) %>%
  kable_styling(full_width = FALSE)
```
...
```{r, echo=FALSE}
print_model(lm_modelOn_UCOTrips_2)
AttractionModel_UCO_On_df<-tidy(lm_modelOn_UCOTrips_2) %>%
  mutate(segment="UCO_On") %>% 
  select(segment,term,estimate)
plot_lm_modelOn_UCOTrips_2 <- apply_lm_modelOn_UCOTrips_2 %>%
    ggplot(aes(W_On_UCOTrips,Predicted)) +
    geom_point()
plot_lm_modelOn_UCOTrips_2
```
...
```{r,echo=FALSE}
print_model(lm_modelOn_UCOTrips_3)
AttractionModel2_UCO_On_df<-tidy(lm_modelOn_UCOTrips_3) %>%
  mutate(segment="UCO_On") %>% 
  select(segment,term,estimate)
plot_lm_modelOn_UCOTrips_3 <- apply_lm_modelOn_UCOTrips_3 %>%
  ggplot(aes(W_On_UCOTrips,Predicted)) +
  geom_point()
plot_lm_modelOn_UCOTrips_3
```
The third model is estimated using only TAZs with UCO trips.
```{r,echo=FALSE}
print_model(selTAZ_lm_modelOn_UCOTrips_2)
plot_lm_modelOn_UCOTrips_2 <- apply_selTAZ_lm_modelOn_UCOTrips_2 %>%
  ggplot(aes(W_On_UCOTrips,Predicted)) +
  geom_point()
plot_lm_modelOn_UCOTrips_2
```
...
```{r,echo=FALSE}
print_model(lm_modelOn_UHOTrips_1)
AttractionModel_UHO_On_df<- tidy(lm_modelOn_UHOTrips_1)%>%
  mutate(segment="UHO_On") %>% 
  select(segment,term,estimate)
```
...
```{r,echo=FALSE}
print_model(selTAZ_lm_modelOn_UHOTrips_1)
...
```{r,echo=FALSE}
print_model(lm_modelOff_UCOTrips_1)
AttractionModel_UCO_Off_df<-tidy(lm_modelOff_UCOTrips_1) %>%
  mutate(segment="UCO_Off") %>% 
  select(segment,term,estimate)
```
Adding distance 

```{r,echo=FALSE}
print_model(lm_modelOff_UCOTrips_2)
AttractionModel2_UCO_Off_df<-tidy(lm_modelOn_UCOTrips_2) %>%
  mutate(segment="UCO_Off") %>% 
  select(segment,term,estimate)
```
...
```{r, echo=FALSE}
print_model(selTAZ_lm_modelOff_UCOTrips_1)
```

#####  UHO Trips by Off-campus students  
UHO trips by off-campus students have two off-campus trip ends. 
```{r, echo=FALSE}
print_model(lm_modelOA_non_1)
AttractionModel_UHO_Off_df<-tidy(lm_modelOA_non_1) %>% 
  mutate(segment="UHO_Off") %>% 
  select(segment,term,estimate)
...
```{r,echo=FALSE}
Attractionmodels_combined_df<-rbind(AttractionModel_UCO_On_df,AttractionModel_UHO_On_df,AttractionModel_UCO_Off_df,
                                    AttractionModel_UHO_Off_df)

Attractionmodels_overview_df<-Attractionmodels_combined_df %>%
  pivot_wider(segment,names_from="term",values_from="estimate")%>%
  kable(digits = 3) %>%
  kable_styling(full_width = FALSE)
Attractionmodels_overview_df

Attractionmodels2_combined_df<-rbind(AttractionModel2_UCO_On_df,AttractionModel2_UCO_Off_df)

Attractionmodels2_overview_df<-Attractionmodels2_combined_df %>%
  pivot_wider(segment,names_from="term",values_from="estimate")%>%
  kable(digits = 3) %>%
  kable_styling(full_width = FALSE)
Attractionmodels2_overview_df
