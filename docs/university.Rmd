...
The model design calls for the following trip purposes: 
...
## Trip Generation  
The first step in the model development process is to create trip production and attraction models for the above trip purposes. This analysis is presented below and relies on a 2014 survey of NCSU students conducted by ITRE on behalf of the NC Department of Transportation. Because we only have survey data for NCSU students, we are assuming that the travel behavior of students attending NCSU is sufficiently representative of NCCU, Duke, and UNC students.  Trip production rates developed based on the NCSU survey were verified using a survey of students attending universities in Virginia.
...
As with the trip production rates, trip attraction rates for off-campus attractions (Home- Other and Campus-Other) were developed based on the survey of NCSU students. For on-campus attractions (Home-based Campus, Campus-to-campus), the attractions are distributed among campus zones proportional to the distribution of the square footage of university buildings. 

To develop off-campus attraction rates, the analysis begins with correlations between the university attractions and land use characteristics at the TAZ level. For trip attractions by on-campus students, the  off-campus student population and the number of retail jobs have the highest correlation coefficients. For trips by off-campus students, off-campus student population has the highest correlation coefficient.  
...
The NCSU survey was used to obtain mode split calibration targets. The graph below shows the mode split by trip purpose for each of the model's trip purposes based on the survey. The tables show the mode split for campus-based trips and for home-based trips, segmented based on the student home location. The modes from the survey were aggregated into 5 categories: Auto, which includes drive and carpool, transit, walk, bike and other 

#### Sample overview
```{r oncampus_modesplit plot, echo = FALSE}
trips_bypurpose_plot + labs(title = "Mode Split by Purpose (except UC1)", 
                                     caption = ("Source: NCSU survey (unweighted)"))
```

The NCSU survey shows that transit is the most common mode for Campus-to-campus trips with 74 percent of these being made by transit. Off-campus students are most likely to travel between home and campus by auto (55 percent), followed by transit (29 percent). On-campus students are most likely to walk (62 percent), followed by transit (19 percent). Almost none of the trips are made by modes classified as "other".
...
Calibration targets were developed for each trip purpose and home location combination above. The calibration targets are the split from the NCSU survey among the model's four modes: walk,bike, transit and auto. The relatively high transit calibration targets will positively affect the forecasted transit ridership.
...
The proposed mode choice coefficients are asserted because the NCSU dataset was not large enough to estimate a mode choice model. The implied value of time (VOT) for walk, bike, transit and drive time is $12 per hour, which is a reasonable average for students. The implied VOT for transit access time is $18 per hour, which is also reasonable.
```{r}
G_IVTT_Coeff <-  -0.04
