...
For simplification, same gamma distribution is used for all university trip purpose, with α, β and θ as 5, 1 and 0.1 respectively. 

The NCSU survey provides information about the length of existing home-based and campus-based trips by NCSU students. The trip start and end location information from the survey was mapped to the model's TAZs. The trip lengths presented in Figures 3 through 5 represent the distance between the centroids of the origin TAZ and the destination TAZ based on the highway skims.  Figure 3 shows that most Home-Campus trips are relatively short distance but that there are some off-campus students who live further away from campus. Home-based trips by off-campus students are longer than home-based trips by on-campus students. Figure 4 shows that campus-based trips tend to be relatively short distance as well.  Figure 5 shows that on-campus trips have both ends are within the same TAZ or within TAZs that are very near to each other. 
These observed trip lengths were used to validate the trip length produced with the gravity model as discussed below in the calibration section.
...
#### Overview trip length distribution from the NCSU survey by trip purpose 
```{r,echo=FALSE}
zonetozonedistance_bypurpose_df %>% 
  kable(digits = 3) %>%
  kable_styling(full_width = FALSE)
```
...
### Trip Generation
Trip generation results compared with the survey are within acceptable parameters as shown by the results of trips per enrollment. 
...
### Trip Distribution
Simulated vs observed data shows similar patterns for the NCSU site as shown below. The other universities display similar pattern.  

#### Cummulative Distance Distribution
