...
```{r,offcampus_modesplit plot, echo = FALSE}
...
```{r}
UG_IVTT_Coeff <-  -0.02
UG_OVTT_Coeff <- UG_IVTT_Coeff * 1.5
UG_Cost_Coeff <-  -0.2  

tibble(Coefficient =c("IVTT","OVTT","Cost"),Value = c(UG_IVTT_Coeff,UG_OVTT_Coeff,UG_Cost_Coeff))%>% 
  kable(digits = 3) %>%
  kable_styling(full_width = FALSE)
```
