...
for these trips. For more information, see the documenation for [Non-Home-Based
Trips](nhb.html).
...
variables and with regard to vehicle ownership.
...
computed using the time coefficient and a value of time of $16.40 per hour.
...
the computation of these variables is described in section below.
...
Auto toll coefficent was able to be estimated in this case and the implied value
of time computed from the time and cost estimates is $16.44/hr, very close to
the desired value of time of $16.40/hr.
...
modes persisted and the relative preference of vehicle insufficient households
...
Larger HHs prefer school pickup/drop-offs compared to riding the bus.
...
For mode choice it was necessary to estimate fair and wait time for AutoPay mode
...
Fare is generally modeled as composed of a flat fee and per mile and per minute
