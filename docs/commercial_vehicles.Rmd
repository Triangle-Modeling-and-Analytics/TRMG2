...
the PM period and far too much in the AM. This skew is made worse considering
...
estimate model parameters. Model parameters were borrowed from the
travel model in Blacksburg, VA and are shown below.
...
tibble(
  ...
  `Averge Distance (mi)` = c(7.3, 9, 10.1)
) %>%
  kable() %>%
  kable_styling(full_width = FALSE)
```
