...
Estimates of workers are only available from the ACS at the tract level. Without
more-disaggregate data to inform the process, the following steps are taken to
assign workers to TAZs:

  1. Calculate the average workers per household for the ACS tract.
  2. Use that average for each TAZ within the tract.
  
In this way, workers are allocated within each tract proportionally based
on the number of households.

Also, it is important to note that ACS tables that count total workers cannot be
used, as they include workers living in group quarters. Instead, total workers 
in households must be imputed from tables that count households by number of
workers.
...
Vehicle data is available from the ACS at the block group level. In addition,
total vehicles can be retrieved for just households. Like workers, however,
the lack of information at the block level means that all TAZs within the same
block group are assigned the same number of vehicles per household. Also, some
block groups had suppressed vehicle information and the regional average of `r round(reg_avg_veh, 2)` was used.
