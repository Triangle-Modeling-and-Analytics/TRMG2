...
add_income_taz <- taz %>%
  ...
  mutate(inc_med = ifelse(
    is.na(inc_med), county_med_inc, inc_med)) %>%
  rename(Median_Inc = inc_med)
...
block groups, the county's median income was used as shown in the following
table.
...
Like vehicles, age data is available from the ACS at the block group level. For
each blockgroup, Caliper computed the percentage of children and seniors.

- Child: Under 18
- Senior: 65 or older

Then, each TAZ within a blockgroup was assigned the same percentages. Age
information was available for all blockgroups with population, and no TAZs
needed to use their county average. This information can be found in the SE
data table in the fields `Pct_Child` and `Pct_Senior`.

```{r, include=FALSE}
calc_age_pct <- acs_bg %>%
  select(GEOID, County, age_child, age_other, age_senior) %>%
  group_by(County) %>%
  mutate(
    cnty_tot = sum(age_child, na.rm = TRUE) + sum(age_other, na.rm = TRUE) +
      sum(age_senior, na.rm = TRUE),
    cnty_age_child = sum(age_child, na.rm = TRUE),
    cnty_age_senior = sum(age_senior, na.rm = TRUE),
    cnty_pct_child = round(cnty_age_child / cnty_tot * 100, 0),
    cnty_pct_senior = round(cnty_age_senior / cnty_tot * 100, 0)
  ) %>%
  mutate(
    total = age_child + age_other + age_senior,
    Pct_Child = ifelse(
      total == 0, cnty_pct_child, round(age_child / total * 100, 0)),
    Pct_Senior = ifelse(
      total == 0, cnty_pct_senior, round(age_senior / total * 100, 0)),
  )
