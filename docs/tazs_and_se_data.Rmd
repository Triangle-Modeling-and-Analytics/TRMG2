...
calc_age_pct <- acs_bg %>%
  ...
  group_by(tract) %>%
  mutate(
    tract_tot = sum(age_child, na.rm = TRUE) + sum(age_other, na.rm = TRUE) +
      sum(age_senior, na.rm = TRUE),
    tract_age_child = sum(age_child, na.rm = TRUE),
    tract_age_senior = sum(age_senior, na.rm = TRUE),
    tract_pct_child = ifelse(
      tract_tot == 0, 0, round(tract_age_child / tract_tot * 100, 0)),
    tract_pct_senior = ifelse(
      tract_tot == 0, 0, round(tract_age_senior / tract_tot * 100, 0))
  ) %>%
  mutate(
    total = age_child + age_other + age_senior,
    Pct_Child = ifelse(
      total == 0, cnty_pct_child, round(age_child / total * 100, 1)),
    Pct_Child = ifelse(Pct_Child < 5, 5, Pct_Child),
    Pct_Senior = ifelse(
      total == 0, cnty_pct_senior, round(age_senior / total * 100, 1)),
...
age_centroids <- taz %>%
  st_point_on_surface() %>%
  st_join(cap_age %>% select(Pct_Child, Pct_Senior)) %>%
  st_drop_geometry() %>%
