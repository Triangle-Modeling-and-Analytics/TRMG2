...
calc_tot_workers <- acs_tract %>%
  mutate(
    ...
    avg_wrk = ifelse(hh_tot == 0, 0, wrk_tot / hh_tot)
  )
...
add_workers_se <- add_income_se %>%
  ...
  select(-avg_wrk) %>%
  relocate(Workers, .after = Median_Inc)
```
...
on the number of households. This information is contained in the `Workers`
field of the SE data table.
...
needed to use a regional average. This information can be found in the SE
data table in the fields `Pct_Child` and `Pct_Senior`.

```{r, include=FALSE}
calc_age_pct <- acs_bg %>%
  select(GEOID, age_child, age_other, age_senior) %>%
  mutate(
    total = age_child + age_other + age_senior,
    Pct_Child = ifelse(total == 0, 0, round(age_child / total * 100, 0)),
    Pct_Senior = ifelse(total == 0, 0, round(age_senior / total * 100, 0)),
  )
...
add_age <- add_workers_se %>%
  ...
  relocate(Pct_Child:Pct_Senior, .after = Workers)
```
