...
Macro "MC" (MacroOpts)
    ...
    for seg in segments do
        ...
        for i = 1 to tables.length do
            ...
            srcOpts = {SourceName: source_name,
                        IDField: source.IDField,
                        JoinSpec: source.JoinSpec}
            
            if source.File <> null then 
              srcOpts.File = source.File
            
            if source.View <> null then 
              srcOpts.View = source.View
            
            obj.AddTableSource(srcOpts)
        end
...
Macro "Disagg MC" (MacroOpts)
    ...
    // Specify outputs
    obj.AddOutputSpec({ChoicesField: MacroOpts.choice_field})
    ret = obj.Evaluate()
    if !ret then
        Throw("Running mode choice model failed for: " + tag)
    obj = null
