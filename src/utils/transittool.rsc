...
dBox "Transit Scenario Comparison" (Args) location: center, center, 80, 20
  ...
  button 19, 15, 30 Prompt:"Generate Results" do 
    ...
    if !RunMacro("TransitScenarioComparison", Args, S2_Dir, TOD) then goto exit	
...
Macro "TransitScenarioComparison" (Args, S2_Dir, TOD) 
    ...
    reporting_dir = S1_Dir + "\\output\\_reportingtool"
    ...
    grouped_vw1 = AggregateTable(
        ...
        {{"Delta_Time", "SUM", }, {"Delta_Trips", "SUM",}}, 
    ...
    //Mapping
    taz_file = Args.TAZs
    mapFile = output_dir + "/Comparison_ExistingAcess_" + TOD + ".map"

    //Create a new, blank map
    {map, {tlyr}} = RunMacro("Create Map", {file: taz_file})
    SetLayerVisibility(map + "|" + tlyr, "false")
    SetLayer(tlyr)
