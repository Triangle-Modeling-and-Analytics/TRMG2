...
output_filename <- paste0(data_dir, "ieei/examine-ieei-flows.csv")
```
...
working_df <- clean_sl_df %>%
  ...
  select(-lat, -lng) %>%
  left_join(., select(socec_df, TAZ_NG, orig_county = County, orig_ADT = ADT), by = c("orig_taz" = "TAZ_NG")) %>%
  left_join(., select(socec_df, TAZ_NG, dest_county = County, dest_ADT = ADT), by = c("dest_taz" = "TAZ_NG"))

```

# Write
```{r write}
write_csv(working_df, output_filename)
```
